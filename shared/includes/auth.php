...
function login_user($user) {
    ...
    $_SESSION['user_role_id'] = $user['role_id'] ?? null; // AJOUTÉ pour permissions
...
function get_current_user_data() {
    ...
    return [
        ...
        'role_id' => $_SESSION['user_role_id'] ?? null, // AJOUTÉ pour permissions
...
function has_role($role) {
    ...
    // Le gestionnaire peut accéder aux fonctions utilisateur
    if ($user_role === 'gestionnaire' && $role === 'utilisateur') {
        return true;
    }
    
    // Compatibility avec anciens noms de rôles
    if ($user_role === 'responsable' && ($role === 'gestionnaire' || $role === 'conducteur')) {
        return true;
    }
    
    if ($user_role === 'conducteur' && $role === 'utilisateur') {
        return true;
    }
...
/**
 * Vérifier si l'utilisateur est administrateur ou gestionnaire
 */
function is_admin_or_responsable() {
    return has_role('admin') || has_role('gestionnaire') || has_role('responsable');
}
...
function can_access_module($module) {
    ...
    $module_access = [
        'admin' => ['vehicules', 'habillement', 'inventaires', 'pharmacie', 'materiel', 'frais', 'admin', 'roles'],
        'gestionnaire' => ['vehicules', 'habillement', 'inventaires', 'pharmacie', 'materiel', 'frais'],
        'responsable' => ['vehicules', 'habillement', 'inventaires', 'pharmacie', 'materiel', 'frais'], // Compatibility
        'utilisateur' => ['vehicules', 'inventaires', 'frais'],
        'conducteur' => ['vehicules', 'inventaires', 'frais'] // Compatibility
    ];
...
/**
 * Forcer l'accès administrateur ou gestionnaire
 */
...
function can_edit_user($target_user_id) {
    ...
    // Seuls les admin et gestionnaires peuvent modifier d'autres utilisateurs
...
function can_access_antenne($antenne_id) {
    ...
    // Les gestionnaires peuvent accéder à leur antenne
    if (is_admin_or_responsable()) {
        return $_SESSION['user_antenne_id'] == $antenne_id;
    }
    
    // Les utilisateurs peuvent accéder à leur antenne
    return $_SESSION['user_antenne_id'] == $antenne_id;
}
...
/**
 * Initialiser la variable globale $current_user
 * Utilisée par les templates et AuthMiddleware
 */
...
// Initialiser automatiquement $current_user si l'utilisateur est connecté
if (is_logged_in()) {
    init_current_user();
}
?>