...
/**
  * Rediriger vers une URL - VERSION CORRIGÉE
 * 
...
function redirect($url, $status_code = 302) {
    ...
    // CORRECTION : Éviter les doubles /v2/v2/
    $url = str_replace('/v2/v2/', '/v2/', $url);
    
    // Ajouter l'URL de base si l'URL est relative
    if (!preg_match('/^https?:\/\//', $url)) {
        // Si l'URL commence déjà par /v2, ne pas rajouter BASE_URL
        if (str_starts_with($url, '/v2/')) {
            $url = 'https://protec64.ovh' . $url;
        } else {
